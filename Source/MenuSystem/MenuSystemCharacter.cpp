...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	SessionSettings->bIsLANMatch = false;		// 禁止本地局域网创建会话(通过互联网连接）
	SessionSettings->NumPublicConnections = 4;	// 允许4个玩家加入游戏
	SessionSettings->bAllowJoinInProgress = true;	// 当游戏会话开始时，允许玩家加入
	SessionSettings->bAllowJoinViaPresence = true;
	// 当Steam设置游戏会话时，他会使用一种状态，它不仅将与世界各地的玩家联系起来，他还会搜索你所在地区的会话
	SessionSettings->bShouldAdvertise = true;	// 允许stream发布会话，以便其他玩家可以找到并加入会话
	SessionSettings->bUsesPresence = true;	  // 允许我按顺序查找正在运行中的会话
	SessionSettings->bUseLobbiesIfAvailable = true;
	// 设置会话相关的键值对
	// 指定匹配类型, 通过在线服务和ping进行广播
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	SessionSearch->MaxSearchResults = 10000;	// 将这个值设置为较大的值有利于搜索到更多会话
	SessionSearch->bIsLanQuery = false;			// 不使用局域网
	// 查找匹配的服务器（下面这行配置可以保证我们查找到的会话都是处于使用状态）
...
void AMenuSystemCharacter::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		FString MatchType;
		Result.Session.SessionSettings.Get(FName("MatchType"), MatchType);	// 拿到会话里的MatchType，填入临时变量MatchType
